...
datum_mit_minus <- function(datum)
  {
  #datum <- c("20220321", "20220322", "20220323", "2022-03-24")
  gsub("(\\d{4})-{0,1}(\\d{2})-{0,1}(\\d{2})", "\\1-\\2-\\3", datum, perl = T)
  }
...
anzahlWorte <- lengths(strsplit(Rtext, " "))
...
gsub("(^|[[:space:]])(\\w)", "\\1\\U\\2", sentence, perl=T)
